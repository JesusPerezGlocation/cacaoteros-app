 ...
 return Scaffold(
  ...
  body: FadeIn(
    child: ListView(
      ...
      children: [
          const TitleSurveysComponents(title:'INFORMACIÓN TÉCNICO - PRODUCTIVA > INFORMACIÓN ESPECÍFICA DEL CACAO' ), 
        ...
        SizedBox(height: size.height*.04),
         //#02
          DropdownComponents(
              ...
              initialValue: '-', 
              hintext: ' Seleccionar dato', 
              items:const ['Barril', 'Cajón','Costal','No fermenta',], 
              validator: (value) => ValidationInputs.inputTypeSelect(value),
              onChanged: (val){   
        ...
        SizedBox(height: size.height*.04),
         ...
         //#04
          DropdownComponents(
              ...
              initialValue: '-', 
              hintext: ' Seleccionar dato', 
              items:const ['Elba', 'Patio cemento','Tierra - Plástico - Costal','Otro'], 
              validator: (value) => ValidationInputs.inputTypeSelect(value),
              onChanged: (val){   
                setState(() => cocoaWhere.text = val.toString());
              },
            ),
        if( cocoaWhere.text.isNotEmpty && cocoaWhere.text =='Otro') SizedBox(height: size.height*.04),
            // #05
        ...
        SizedBox(height: size.height*.04),
         //#06
          DropdownComponents(
              ...
              items:const ['Asociaciones', 'Cooperativas','Intermediario','Transf. local'], 
              validator: (value) => ValidationInputs.inputTypeSelect(value),
              onChanged: (val){   
                setState(() => cocoaWhere.text = val.toString());
              },
      ...
      if( selfConsumption.text.isNotEmpty && selfConsumption.text =='Si') SizedBox(height: size.height*.04),
            // #08
      ...
      SizedBox(height: size.height*.04),
       ...
       SizedBox(height: size.height*.04),
          //#15
        InputsComponent(
          title: 'Número de árboles de cacao para renovar por injerto en leño grueso', 
